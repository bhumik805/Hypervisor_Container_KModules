...
uint8_t num_exit=0;
uint32_t num_exit_by_in=0;
uint32_t num_exit_by_out=0;
...
int run_vm(struct vm *vm, struct vcpu *vcpu, size_t sz)
{
	...
	for (;;)
	{
		...
		case KVM_EXIT_IO:
		{
			...
			{
				uint32_t *receivedStringPointer = (uint32_t *)((char *)vcpu->kvm_run + vcpu->kvm_run->io.data_offset);

				printf("Received string: %s\n", *(receivedStringPointer+sz));

				// Continue processing or handle other VM exits
				continue;
			}
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_IN && vcpu->kvm_run->io.port == 0xEC)
			{
				printf("Received num_exit: %u\n", num_exit);
				vcpu->kvm_run->io.data_offset = num_exit;
				continue;
			}
