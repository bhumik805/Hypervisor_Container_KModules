...
run()
{
    ...
    # mkdir $CONTAINERDIR/$NAME/rootfs/dev
    ...
    unshare --pid --mount --net --ipc --uts --kill-child chroot $CONTAINER_ROOTFS /bin/bash -c "chmod 755 / && mount -t sysfs sysfs /sys && mount -t proc proc /proc  && $INIT_CMD_ARGS"
}
...
exec()
{
    ...
    [ -z "$UNSHARE_PID" ] && die "Cannot find container process1"
    ...
    CIP=$CONTAINER_INIT_PID
    ECA=$EXEC_CMD_ARGS
    nsenter --target $CIP --pid --mount --uts --ipc --net -w -r $ECA
}
...
addnetwork()
{
    ...
    IPEER=$INSIDE_PEER
    OPEER=$OUTSIDE_PEER
    ip link add $IPEER type veth peer name $OPEER
    ip link set dev $IPEER netns $NS
    ...
    ip -n $NS link set $IPEER up
    ip link set $OPEER up
