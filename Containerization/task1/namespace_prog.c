...
int main() {
   ...
   // ------------------ WRITE CODE HERE ------------------
    ...
    if((child_pid=clone(&child_function,&child_stack,CLONE_NEWUTS | CLONE_NEWPID | SIGCHLD,pipefd))==-1){
            printf("clone error");   
    } 
    ...
    */
    ...
    // ------------------ WRITE CODE HERE ------------------
    char pidn_amespace_[100];
    sprintf(pidn_amespace_,"/proc/%d/ns/pid",child_pid);
    char *pidnamespacetemp=pidn_amespace_;
    int pidnamespacefd_ = open(pidnamespacetemp,O_RDONLY);
    if(setns(pidnamespacefd_,CLONE_NEWPID)==-1){
        printf("couldnt set pid ns");
    }
    ...
    if (fork() == 0) {
        ...
        */
        ...
        char utsnames_pacefname_[100];
        sprintf(utsnames_pacefname_,"/proc/%d/ns/uts",child_pid);
        int utsname_spacefd_ = open(utsnames_pacefname_,O_RDONLY);
        if(setns(utsname_spacefd_,CLONE_NEWUTS)==-1){
            printf("error while setting pid ns");
        }
